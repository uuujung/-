...
#ys_pickle_vsadd
features = ['OLR(kg VS/m3)', 'kg_VSadd', 'VFA', 'Alkalinity', 'VFA/Alk ratio', 
            'Biogas Production(m3 x 4)', 'Biogas Yield_Vsadd(Nm3/kg Vsadd)',
            'vsadd_mean','new_yield','new_Vsadd']
...
#date = 1
    ...
    for iepoch, train_data in enumerate(train_dataset.take(train_take * 500)): #
        ...
        with tf.GradientTape() as tape:
            ...
            loss = 0.5 * tf.reduce_mean(tf.reduce_sum(tf.abs((in_key_outputs - keys) / keys) * tf.constant([1. / 3, 1. / 3., 1. / 3.]), -1)) * 100. \
...
#tf.reduce_mean(tf.abs((in_key_outputs - keys)/keys) * tf.constant([1. / 3, 1. / 3., 1. / 3.]))*100
    ...
    if date >= warm_up:
        ...
        jackknife = kde.b_jackknife(start = 0.1, step = 5)
        ...
        k_nw_intp_data_ = pd.DataFrame(nw_intp_data, columns = ['RegDate', 'VFA', 'Alkalinity', 'VFA/Alk ratio']) #원데이터 + 보간 값
        ...
        k_nw_intp_data_ = k_nw_intp_data_[np.newaxis, ...]
        ...
        jackknife_y = kde_y.b_jackknife(start = 0.1, step = 5)
        ...
        y_nw_intp_data_ = y_nw_intp_data_[np.newaxis, ...]        
        ...
        k_pred_for_i_ = k_nw_intp_data_[:, -window_size:, :]
           
            #y_pred_for_i_ = y_pred_for_i_[:, -window_size:, :]
        y_pred_for_i_ = y_nw_intp_data_[:, -window_size:, :]
...
'''
tprod = [t[0] for t in true_]
pprod = [p[0] for p in pred_]
    
plt.plot(list(range(0,test_period)), tprod)
plt.plot(list(range(0, test_period)), pprod, 'r')
plt.title('VFA')
plt.show()
    
    
tprod = [t[1] for t in true_]
pprod = [p[1] for p in pred_]
...
tprod = [t[2] for t in true_]
pprod = [p[2] for p in pred_]
...
tprod = [t[3] for t in true_]
pprod = [p[3] for p in pred_]
...
tprod = [t[4] for t in true_]
pprod = [p[4] for p in pred_]
...
############################################그래프
#kde 추세값
#kde_g = KDE.KDE(df.drop('index', axis=1))
...
#예측값
pprod_vfa = pd.DataFrame([p[0] for p in pred_], columns = ['vfa'])
pprod_alk = pd.DataFrame([p[1] for p in pred_], columns = ['alk'])
pprod_vfa_alk = pd.DataFrame([p[2] for p in pred_], columns = ['vfa/alk'])
pprod_production = pd.DataFrame([p[3] for p in pred_], columns = ['production'])
#pprod_yield = pd.DataFrame([p[4] for p in pred_], columns = ['yield'])
#pprod_new_yield = pd.DataFrame([p[5] for p in pred_], columns = ['new_yield'])
pprod_new_yield = pd.DataFrame([p[4] for p in pred_], columns = ['new_yield'])
...
bottom.plot(nw_intp_data_g2[:-6][features[2]], color = 'gray') #나다라야 보간값 선
bottom.plot(nw_data_g2[:-6][[features[2]]], color = 'dodgerblue')#나다라야 추세
bottom.scatter(pprod_vfa.index,pprod_vfa, 10 ,color = 'red') # 예측값 산점도
bottom.plot(pprod_vfa, color = 'red') #예측값 선
...
bottom.plot(nw_intp_data_g2[:-6][features[3]], color = 'gray') #나다라야 보간값 선
bottom.plot(nw_data_g2[:-6][[features[3]]], color = 'dodgerblue')#나다라야 추세
bottom.scatter(pprod_alk.index,pprod_alk, 10 ,color = 'red') # 예측값 산점도
bottom.plot(pprod_alk, color = 'red') #예측값 선
...
bottom.plot(nw_intp_data_g2[:-6][features[5]], color = 'gray') #나다라야 보간값 선
bottom.plot(nw_data_g2[:-6][[features[5]]], color = 'dodgerblue')#나다라야 추세
bottom.scatter(pprod_production.index, pprod_production, 10 ,color = 'red') # 예측값 산점도
bottom.plot(pprod_production, color = 'red') #예측값 선
...
#col_mean = pd.DataFrame(np.mean(mape_,axis=0), index=['vfa','alk','vfa/alk','production','yield','new_yield'])
