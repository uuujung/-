...
#import model7_scaled_residual2 as Model
...
#tf.reduce_mean(tf.abs((in_key_outputs - keys)/keys) * tf.constant([1. / 3, 1. / 3., 1. / 3.]))*100
    ...
    if date >= warm_up:
        ...
        for f in range(-gap, 0, 1):
            ...
            #k_pred_for_i_= k_pred_for_i_.append(A)
            #y_pred_for_i_= y_pred_for_i_.append(B)
            
            #start_k = datetime.datetime(2020, 1, 28)
            #Time_k = list()
            #for i in range(len(k_pred_for_i_)):
            #    Time_k.append((start_k + datetime.timedelta(days = i)).strftime('%Y-%m-%d'))
     
            #k_pred_for_i_['Time'] = Time_k
            #k_pred_for_i_.index = k_pred_for_i_['Time']
            #k_pred_for_i_ = k_pred_for_i_.drop(['Time'], axis=1) 
            #k_pred_for_i_ = k_pred_for_i_.values
            #k_pred_for_i_ = k_pred_for_i_[np.newaxis, ...] 
        
        
            #y_pred_for_i_['Time'] = Time_k
            #y_pred_for_i_.index = y_pred_for_i_['Time']
            #y_pred_for_i_ = y_pred_for_i_.drop(['Time'], axis=1)      
            ...
            #랜덤
            #=================================================================
            #num = random.randint(0, 184 + f )
            
            #k_pred_for_i_ = k_pred_for_i_.append(k_pred_for_i_[num : num + 1])
            ...
            #y_pred_for_i_ = y_pred_for_i_.append(y_pred_for_i_[num : num + 1])
            ...
            #k_pred_for_i_ = k_pred_for_i_.append(k_mean)
            ...
            #y_pred_for_i_ = y_pred_for_i_.append(y_mean)
            ...
            #nw 대체
            #=================================================================
            #k_pred_for_i_ = k_train_for_i.reset_index().rename(columns={"Time": "RegDate"})

            #kde = KDE.KDE(k_pred_for_i_)
            #jackknife = kde.b_jackknife(start = 0.1, step = 10)
            #num_bins = kde.num_bins(jackknife, start = 1, stop = 50, step = 5)
        
            #NW_data = kde.NW_predict_test(num_bins, jackknife) #num_bins, b_jackknife
            #nw_data, nw_intp_data = kde.NW_interpolation(NW_data)
        
            #k_nw_data = pd.DataFrame(nw_data, columns = ['RegDate',  'NH4_N', 'VFA', 'Alkalinity', 'PH'])#, 'VFA/Alk ratio']) #원데이터 + 보간 값
        
            #k_pred_for_i_ = pd.concat((k_pred_for_i_, k_nw_data[-1:]))
            ...
            #k_nw_data_ = k_pred_for_i_.drop(['RegDate', 'Time'], axis=1)
            #k_nw_data_ = k_nw_data_ .values.astype(np.float32) 
            #k_pred_for_i_ = k_nw_data_[np.newaxis, ...]
                
                

            #y_pred_for_i_ = y_train_for_i.reset_index().rename(columns={"Time": "RegDate"})
            
            #kde_y = KDE.KDE(y_pred_for_i_)
            #jackknife_y = kde_y.b_jackknife(start = 0.1, step = 10)
            #num_bins_y = kde_y.num_bins(jackknife_y, start = 1, stop = 50, step = 5)
            
            #NW_data_y = kde_y.NW_predict_test(num_bins_y, jackknife_y) #num_bins,b_jackknife
            #nw_data_y, nw_intp_data_y = kde_y.NW_interpolation(NW_data_y)
            
            #y_nw_data = pd.DataFrame(nw_data_y, columns = ['RegDate', 'Biogas Production(m3 x 4)'])#, 'new_yield'])#원데이터 + 보간 값
            
            #y_pred_for_i_ = pd.concat((y_pred_for_i_, y_nw_data[-1:]))
            
            #y_pred_for_i_['Time'] = Time_k
            #y_pred_for_i_.index = y_pred_for_i_['Time']
            #y_nw_data_ = y_pred_for_i_.drop(['RegDate', 'Time'], axis=1)
            #y_nw_data_ = y_nw_data_ .values.astype(np.float32) 
            #y_pred_for_i_ = y_nw_data_[np.newaxis, ...]       
            ...
            #k_pred_for_i_ = k_pred_for_i_[:, -window_size:-1, :]
            #print(np.median(k_pred_for_i_, 1))
            #k_pred_for_i_ = np.concatenate((k_pred_for_i_,  np.median(k_pred_for_i_, 1)[np.newaxis, ...]), 1)
            #y_pred_for_i_ = y_pred_for_i_[:, -window_size:-1, :]
            #y_pred_for_i_ = np.concatenate((y_pred_for_i_,  np.median(y_pred_for_i_, 1)[np.newaxis, ...]), 1)


            # data for prediction
            
            if (f + 1) != 0:
                x_pred_for_i = x_data_for_i.iloc[- (window_size - f - 1):(f + 1)]
                kt_pred_for_i = kt_data_for_i.iloc[- (window_size - f - 1):(f + 1)]
            else:
                x_pred_for_i = x_data_for_i.iloc[- (window_size - f - 1):]            
                kt_pred_for_i = kt_data_for_i.iloc[- (window_size - f - 1):]            
            ...
            k_pred_for_i_ = np.concatenate((k_pred_for_i_[:, :-1, :], pred_k), 1)
            k_append = pd.DataFrame( pred_k.reshape(1,4), columns = [ 'NH4_N', 'VFA', 'Alkalinity', 'PH'])
            k_train_for_i = k_train_for_i.append(k_append* k_train_std_for_i + k_train_mean_for_i)

    
            y_pred_for_i_ = np.concatenate((y_pred_for_i_[:, :-1, :], pred_y), 1)
            y_append = pd.DataFrame( pred_y.reshape(1,1), columns = ['Biogas Production(m3 x 4)'])
            y_train_for_i = y_train_for_i.append(y_append* y_train_std_for_i + y_train_mean_for_i)
            
            #NW================================================================nw
            #start_k = datetime.datetime(2020, 1, 28)
            #Time_k = list()
            #for i in range(len(k_train_for_i)):
            #    Time_k.append((start_k + datetime.timedelta(days = i)).strftime('%Y-%m-%d %H:%M:%S'))
                
            #k_train_for_i['Time'] = Time_k
            #k_train_for_i.index = k_train_for_i['Time']
            #k_train_for_i = k_train_for_i.drop(['Time'], axis=1)
        
            #y_train_for_i['Time'] = Time_k
            #y_train_for_i.index = y_train_for_i['Time']
            #y_train_for_i = y_train_for_i.drop(['Time'], axis=1)
            #=================================================================
...
#plt.scatter(Z[:-8], Y[:,5][:-8], 15, color = 'grey', label = 'data') #원데이터 산점도
#plt.plot(Z[:-8], Y[:,5][:-8], color = 'gray')
#plt.plot(df2[:-6][[features[5]]]) #인터폴레이션
...
top.set_xlabel('Time')
top.set_ylabel('VFA/Alk ratio')
top.set_title('VFA/Alk ratio')
...
top.plot(nw_intp_data_g2[:-6][features[4]][sub1_Left:sub1_Right], color = 'gray') #나다라야 보간값 선
top.plot(nw_data_g2[:-6][[features[4]]][sub1_Left:sub1_Right], color = 'dodgerblue')#나다라야 추세
top.scatter(pprod_vfa_alk.index, pprod_vfa_alk, 10 ,color = 'red') # 예측값 산점도
top.plot(pprod_vfa_alk, color = 'red') #예측값 선

#bottom
bottom = fig.add_subplot(2,2,(3,4))
bottom.set_xlabel('Time' )
bottom.set_ylabel('VFA/Alk ratio' )
bottom.set_title('VFA/Alk ratio')
bottom.scatter(nw_intp_data_g2.index[:-6], nw_intp_data_g2[:-6][features[4]], 15 ,color = 'gray') #나다라야 보간값 산점도
bottom.plot(nw_intp_data_g2[:-6][features[4]], color = 'gray') #나다라야 보간값 선
bottom.plot(nw_data_g2[:-6][[features[4]]], color = 'dodgerblue')#나다라야 추세
bottom.scatter(pprod_vfa_alk.index, pprod_vfa_alk, 10 ,color = 'red') # 예측값 산점도
bottom.plot(pprod_vfa_alk, color = 'red') #예측값 선


bottom.fill_between((sub1_Left,sub1_Right), 0.008, 0.07, facecolor='green', alpha=0.2)
con1 = patches.ConnectionPatch(xyA=(sub1_Left, 0.018), coordsA=top.transData, 
                       xyB=(sub1_Left, 0.07), coordsB=bottom.transData, color = 'green')
fig.add_artist(con1)
con2 = patches.ConnectionPatch(xyA=(sub1_Right, 0.018), coordsA=top.transData, 
                       xyB=(sub1_Right, 0.07 ), coordsB=bottom.transData, color = 'green')
fig.add_artist(con2)


#plt.scatter(Z[:-8], Y[:,4][:-8], 15, color = 'grey', label = 'data') #원데이터 산점도
#plt.plot(Z[:-8], Y[:,4][:-8], color = 'gray')
#plt.plot(df2[:-6][[features[4]]]) #인터폴레이션
...
top.set_xlabel('Time')
top.set_ylabel('new_yield')
top.set_title('new_yield')

sub1_Left = 249
sub1_Right = 278

top.scatter(nw_intp_data_g2.index[:-6][sub1_Left:sub1_Right], nw_intp_data_g2[:-6][features[8]][sub1_Left:sub1_Right], 15 ,color = 'gray') #나다라야 보간값 산점도
top.plot(nw_intp_data_g2[:-6][features[8]][sub1_Left:sub1_Right], color = 'gray') #나다라야 보간값 선
top.plot(nw_data_g2[:-6][[features[8]]][sub1_Left:sub1_Right], color = 'dodgerblue')#나다라야 추세
top.scatter(pprod_new_yield.index, pprod_new_yield, 10 ,color = 'red') # 예측값 산점도
top.plot(pprod_new_yield, color = 'red') #예측값 선

#bottom
bottom = fig.add_subplot(2,2,(3,4))
bottom.set_xlabel('Time' )
bottom.set_ylabel('new_yield' )
bottom.set_title('new_yield')
bottom.scatter(nw_intp_data_g2.index[:-6], nw_intp_data_g2[:-6][features[8]], 15 ,color = 'gray') #나다라야 보간값 산점도
bottom.plot(nw_intp_data_g2[:-6][features[8]], color = 'gray') #나다라야 보간값 선
bottom.plot(nw_data_g2[:-6][[features[8]]], color = 'dodgerblue')#나다라야 추세
bottom.scatter(pprod_new_yield.index, pprod_new_yield, 10 ,color = 'red') # 예측값 산점도
bottom.plot(pprod_new_yield, color = 'red') #예측값 선


bottom.fill_between((sub1_Left,sub1_Right), 0.1, 0.8, facecolor='green', alpha=0.2)
con1 = patches.ConnectionPatch(xyA=(sub1_Left, 0.13), coordsA=top.transData, 
                       xyB=(sub1_Left, 0.8), coordsB=bottom.transData, color = 'green')
fig.add_artist(con1)
con2 = patches.ConnectionPatch(xyA=(sub1_Right, 0.13), coordsA=top.transData, 
                       xyB=(sub1_Right,0.8), coordsB=bottom.transData, color = 'green')
fig.add_artist(con2)
